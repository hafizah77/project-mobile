import 'dart:convert'; // # Untuk mengubah data JSON (khusus Firebase Realtime DB)
...
import 'package:travel_app/page/widgets/bottomNavbar.dart'; // # Widget bottom navbar

// # Stateful widget karena data riwayat akan berubah (diambil dari Firebase dan Realtime DB)
...
class _RiwayatAdminPageState extends State<RiwayatAdminPage> {
  final String baseUrl = 'https://travel-app-e27ba-default-rtdb.firebaseio.com'; // # URL dasar Realtime DB

  List<Map<String, dynamic>> _aktivitas = []; // # Menyimpan semua log aktivitas

  @override
  void initState() {
    super.initState();
    _loadAktivitas(); // # Load data log saat halaman dimulai
  }

  // # Fungsi utama untuk mengambil dan menyimpan semua log ke _aktivitas
  Future<void> _loadAktivitas() async {
    final data = await _fetchAktivitas(); // # Ambil data dari berbagai sumber
    setState(() {
      _aktivitas = data;
    });
  }

  // # Fungsi gabungan untuk fetch semua jenis log dari Firestore & Realtime DB
  Future<List<Map<String, dynamic>>> _fetchAktivitas() async {
    ...
    try {
      ...
      for (var doc in transaksiLogDocs.docs) {
        aktivitas.add({
          ...
          'detail': 'Transaksi ke ${doc['destination_name']} sebanyak ${doc['ticket_count']} tiket (Rp${doc['total_price']})',
    ...
    // # Ambil data registrasi dari Realtime Database
    try {
  ...
  // # Fungsi untuk menampilkan ikon berbeda sesuai jenis aktivitas
  ...
  // # Fungsi untuk menghapus log berdasarkan sumber
  ...
  // # Fungsi untuk edit log hanya di Firestore (bukan Realtime DB)
  ...
  // # Tambahkan log baru ke koleksi "logins"
  void _addLog() {
  ...
  // # Format tampilan waktu (dd-mm hh:mm)
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            ...
            onPressed: _addLog, // # Tombol tambah log
          ),
        ],
      ),
      backgroundColor: Colors.grey[100],
      bottomNavigationBar: const BottomNavbar(currentIndex: 1), // # Navigasi bawah
      body: _aktivitas.isEmpty
          ? const Center(child: CircularProgressIndicator()) // # Loading spinner saat data di-load
          : ListView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                return Card(
                  ...
                  child: ListTile(
                    leading: _getIconByType(item['type']), // # Icon berdasarkan tipe log
                    title: Text(item['detail']), // # Isi detail log
                    subtitle: Text(item['email']), // # Email pengguna
                    trailing: Row(
                      mainAxisSize: MainAxisSize.min,
                      children: [
                        Text(
                          _formatDate(item['timestamp']), // # Tanggal & waktu
